...
for VOLUME in 805306368 1073741824
do
...
do
    date
    echo "Berechne: [$HOSTS]-$NODES $CHUNK $VOLUME C$CSIZE"

    # logging
    LOGSTRING=`hostname | tr -d "\n" ; echo -n " - "; date | tr -d "\n"`

    mpirun -H $HOSTLIST -n $NODES $BIN -c $CHUNK -v $VOLUME -C $CSIZE -m >>"data/$HOSTS-$NODES-$VOLUME-$CHUNK-$CSIZE.log"
